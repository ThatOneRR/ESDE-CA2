        ...
        pool.getConnection((err, connection) => {
            if (err) {
                ...
                connection.query('INSERT INTO user ( fullname, email, user_password, role_id) VALUES (?,?,?,2) ', [fullname, email, password], (err, rows) => {
                    if (err) {
                        reject(createJSONResponse('Unable to register user', Status.ERROR, null));
                    } else {
                        resolve(rows);
                    }
                    connection.release();
        ...
        pool.getConnection((err, connection) => {
            if (err) {
                ...
                connection.query(`UPDATE user SET role_id =${newRoleId} WHERE user_id=${recordId}`, (err, rows) => {
                    if (err) {
                        reject(createJSONResponse('Unable to update user', Status.ERROR, null));
                    } else {
                        resolve(rows);
                    }
                    connection.release();
...
module.exports.getUserData = (pageNumber, search) => {
    ...
    if ((search == '') || (search == null)) {
        ...
        userDataQuery = `SELECT user_id, fullname, email, role_name FROM user INNER JOIN role ON user.role_id = role.role_id LIMIT ${limit} OFFSET ${offset}; SET @total_records = (SELECT count(user_id) FROM user); SELECT @total_records total_records; `;
    } else {
        userDataQuery = `SELECT user_id, fullname, email, role_name FROM user INNER JOIN role ON user.role_id = role.role_id AND fullname LIKE '%${search}%' LIMIT ${limit} OFFSET ${offset}; SET @total_records = (SELECT count(user_id) FROM user WHERE fullname LIKE '%${search}%'); SELECT @total_records total_records;`;
    }
        ...
        pool.getConnection((err, connection) => {
            if (err) {
                ...
                connection.query(userDataQuery, [search, offset, limit], (err, results) => {
                    if (err) {
                        reject(createJSONResponse('Unable to get user data', Status.ERROR, null));
                    } else {
...
module.exports.getOneUserData = function (recordId) {
    ...
    userDataQuery = `SELECT user_id, fullname, email, user.role_id, role_name FROM user INNER JOIN role ON user.role_id = role.role_id WHERE user_id=` + recordId;
    ...
    return new Promise((resolve, reject) => {
        ...
        pool.getConnection((err, connection) => {
            if (err) {
                ...
                connection.query(userDataQuery, (err, results) => {
                    if (err) {
                        reject(createJSONResponse('Unable to get design data', Status.ERROR, null));
                    } else {
                        resolve(results);
                    }
                    connection.release();
        ...
        pool.getConnection((err, connection) => {
            if (err) {
                ...
                connection.query(`UPDATE file SET design_title ='${title}' , design_description='${description}' WHERE file_id=${recordId}`, (err, rows) => {
                    if (err) {
                        reject(createJSONResponse('Unable to update design', Status.ERROR, null));
                    } else {
                        resolve(rows);
                    }
                    connection.release();
